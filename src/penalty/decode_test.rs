//! Decode the built-in watchtower's SUCCESSFUL penalty tx to learn the exact
//! structure our executor must reproduce.
...
mod tests {
    ...
    fn decode_working_penalty() {
        ...
        println!("WORKING sig verifies: C(no-prefix)={vc} B(len-prefix)={vb}");
        println!("odata0 len = {}, version = {}", odata0.len(), u64::from_be_bytes(version.try_into().unwrap()));
    }
}
